...
class Template21 extends AbstractParser{
    ...
    public function getCity(){
        ...
        $content = file_get_contents($dir.'/cityzhilian.txt');
        $cityArr = json_decode($content,true);
        $this->city = $cityArr;
    }
        ...
        if($json){
            ...
            if($cityArr){
                ...
                foreach($cityArr as $key=>$value){
                    if($this->city[$this->city[$value]['fid']]){
                        if(!in_array($this->city[$this->city[$value]['fid']]['name'],$cityNameArr)){
                            $cityNameArr[] = $this->city[$this->city[$value]['fid']]['name'];
                        }
                    }else{
                        if(!in_array($this->city[$value]['name'],$cityNameArr)){
                            $cityNameArr[] = $this->city[$value]['name'];
                        }
                    }
                }
            ...
            $data['birth_year'] = $candidate['birthYear'];
            if($detail['Gender']==1){
                $data['sex'] = '男';
